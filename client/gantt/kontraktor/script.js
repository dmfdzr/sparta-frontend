...
async function saveProjectSchedule(statusType = "Active") {
    ...
    currentTasks.forEach((task) => {
        ...
        payload[`Keterlambatan_Kategori_${task.id}`] = "0" // This might be overwritten or might need to be calculated differently
        }
    })
    ...
    currentTasks.forEach((task) => {
        ...
        const startDay = Number.parseInt(range.start) || 0
        const endDay = Number.parseInt(range.end) || 0
...
function applyTaskSchedule(silentMode = false) {
    ...
    for (const task of currentTasks) {
        ...
        if (endDay < startDay) {
            alert(
            `Error pada ${task.name}: Hari selesai (${endDay}) tidak boleh lebih kecil dari hari mulai (${startDay})!`,
            )
            hasError = true
            return
        }
        ...
        updatedTasks.push({
        ...task,
        start: minStart === Number.POSITIVE_INFINITY ? 0 : minStart,
        duration: totalDuration,
        inputData: { ranges },
        })
    }
...
function renderChart() {
    ...
    const totalDaysToRender = Math.max(currentProject.work === "ME" ? totalDaysME : totalDaysSipil, maxTaskEndDay + 10)
    ...
    currentTasks.forEach((task) => {
        ...
        const propagatedDelay = calculatePropagatedDelay(task.id)
        ...
        const totalDisplayDelay = displayDelay + propagatedDelay

        html += '<div class="task-row">'
        html += `<div class="task-name">
                <span>${task.name}</span>
                <span class="task-duration">
                    Total Durasi: ${totalDuration} hari
                    ${displayDelay > 0 ? `<span style="color: #e53e3e;">(+${displayDelay} hari delay)</span>` : ""}
                    ${propagatedDelay > 0 ? `<span style="color: #cc5500;">(+${propagatedDelay} hari propagated)</span>` : ""}
                </span>
            </div>`
        ...
        if (propagatedDelay > 0 && ranges.length > 0) {
        const lastRange = ranges[ranges.length - 1]
        const lastEnd = new Date(projectStartDate)
        lastEnd.setDate(projectStartDate.getDate() + lastRange.end - 1)

        const propagatedLeftPos = (lastRange.end + displayDelay) * DAY_WIDTH
        const propagatedWidthPos = propagatedDelay * DAY_WIDTH - 1
        const tEndWithAllDelay = new Date(lastEnd)
        tEndWithAllDelay.setDate(lastEnd.getDate() + displayDelay + propagatedDelay)

        html += `<div class="bar delayed" data-task-id="${task.id}-propagated"
                        style="left: ${propagatedLeftPos}px; width: ${propagatedWidthPos}px; box-sizing: border-box; background: linear-gradient(135deg, #cc5500 0%, #994400 100%); opacity: 0.7;"
                        title="Keterlambatan Propagasi ${task.name}: +${propagatedDelay} hari (s/d ${formatDateID(tEndWithAllDelay)})">
                    +${propagatedDelay}
                </div>`
        }
...
function drawDependencyLines() {
    ...
    currentTasks.forEach((task) => {
        ...
        task.dependencies.forEach((depId) => {
            ...
            if (fromTask && toTask) {
                // Find the last bar of the from task
                const fromRanges = fromTask.inputData?.ranges || []
                let fromBar = null
                if (fromRanges.length > 0) {
                // Find the range with the latest end date to draw the line from its end
                const lastRange = fromRanges.reduce((prev, curr) => (prev.end > curr.end ? prev : curr))
                fromBar = document.querySelector(
                    `.bar[data-task-id="${fromTask.id}-${fromRanges.findIndex((r) => r === lastRange)}"]`,
                )
                }

                // Find the first bar of the to task
                const toRanges = toTask.inputData?.ranges || []
                let toBar = null
                if (toRanges.length > 0) {
                toBar = document.querySelector(`.bar[data-task-id="${toTask.id}-0"]`)
                }
...
function renderDependenciesForm() {
    ...
    const formContainer = document.getElementById("dependenciesForm")

    let html = '<div class="dependency-container">'

    currentTasks.forEach((task) => {
        const deps = task.dependencies || []
        html += `
                <div style="margin-bottom: 15px; padding: 10px; background: #f8f9fa; border-radius: 6px;">
                    <div style="font-weight: 600; color: #1a202c; margin-bottom: 10px; font-size: 14px;">
                        📌 ${task.name}
                    </div>
                    <div id="deps-${task.id}" class="dependency-inputs" style="margin-bottom: 8px;">
            `

        if (deps.length === 0) {
        html += '<div class="dependency-row" style="margin-bottom: 0;">Tidak ada ketergantungan</div>'
        } else {
        deps.forEach((depId, idx) => {
            ...
            html += `
                        <div class="dependency-row">
                            <div style="flex: 1;">
                                <select class="dependency-select" onchange="updateDependency(${task.id}, ${idx}, this.value)">
                                    <option value="">-- Hapus Ketergantungan --</option>
                                    ${currentTasks
                                    .map((t) =>
                                        t.id !== task.id
                                        ? `<option value="${t.id}" ${depId === t.id ? "selected" : ""}>${t.name}</option>`
                                        : "",
                                    )
                                    .join("")}
                                </select>
                            </div>
                            <button class="dependency-remove" onclick="removeDependency(${task.id}, ${idx})">−</button>
                        </div>
                    `
        })
        }
        ...
        html += `
                    ...
                    <button class="btn-add-dependency" onclick="addDependency(${task.id})">
...
function addDependency(taskId) {
    ...
    // Add empty dependency (user needs to select)
    task.dependencies.push(null)
    renderDependenciesForm()
}

function removeDependency(taskId, depIndex) {
    const task = currentTasks.find((t) => t.id === taskId)
...
function updateDependency(taskId, depIndex, depTaskId) {
    const task = currentTasks.find((t) => t.id === taskId)
    if (!task || !task.dependencies) return

    if (depTaskId === "") {
        // Remove dependency if user selected "Hapus"
        task.dependencies.splice(depIndex, 1)
    } else {
        const depId = Number.parseInt(depTaskId, 10)
        if (!isNaN(depId) && depId !== taskId) {
        task.dependencies[depIndex] = depId
        }
    }
    renderDependenciesForm()
    if (hasUserInput) {
...
// Calculate propagated delay based on dependencies
function calculatePropagatedDelay(taskId, memo = {}) {
    ...
    if (task.dependencies && task.dependencies.length > 0) {
        ...
        if (depId !== null && depId !== undefined) {
            const depTask = currentTasks.find((t) => t.id === depId)
            if (depTask) {
            // Get delay from dependency task
            const depRanges = depTask.inputData?.ranges || []
            const depDelay = depRanges.reduce((sum, r) => sum + (r.keterlambatan || 0), 0) || depTask.keterlambatan || 0

            // Recursively get delays from dependencies of dependencies
            const cascadingDelay = calculatePropagatedDelay(depId, memo)
            const totalDepDelay = depDelay + cascadingDelay

            if (totalDepDelay > maxDependencyDelay) {
                maxDependencyDelay = totalDepDelay
            }
            }
        }
        })
    ...
    memo[taskId] = maxDependencyDelay
    return maxDependencyDelay
}
...
// Apply schedule changes and update dependencies/chart
function applySchedule() {
    const totalDuration = currentTasks.reduce((acc, t) => acc + t.duration, 0)
    if (totalDuration === 0) {
        alert("⚠️ Jadwal masih kosong. Mohon isi hari mulai dan hari selesai untuk setidaknya satu tahapan.")
        return
    }

    // Validate that input data exists
    const hasValidInputData = currentTasks.some((t) => t.inputData && t.inputData.ranges && t.inputData.ranges.length > 0)

    if (!hasValidInputData) {
        alert("⚠️ Silakan isi jadwal pengerjaan untuk tahapan terlebih dahulu.")
        return
    }

    if (hasCircularDependency()) {
        alert("⚠️ Ketergantungan sirkular terdeteksi! Pastikan tidak ada siklus dalam ketergantungan antar tahapan.")
        return
    }
